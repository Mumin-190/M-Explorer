...
import me.mumin.android.files.compat.PreferenceManagerCompat
import me.mumin.android.files.compat.getDescriptionCompat
import me.mumin.android.files.compat.readBooleanCompat
import me.mumin.android.files.compat.writeBooleanCompat
import me.mumin.android.files.compat.writeParcelableListCompat
import me.mumin.android.files.file.DocumentTreeUri
import me.mumin.android.files.file.asExternalStorageUriOrNull
import me.mumin.android.files.file.displayName
import me.mumin.android.files.file.storageVolume
import me.mumin.android.files.filelist.FileSortOptions
import me.mumin.android.files.navigation.BookmarkDirectory
import me.mumin.android.files.navigation.StandardDirectorySettings
import me.mumin.android.files.provider.archive.ArchiveFileSystem
import me.mumin.android.files.provider.common.ByteString
import me.mumin.android.files.provider.common.moveToByteString
import me.mumin.android.files.provider.content.ContentFileSystem
import me.mumin.android.files.provider.document.DocumentFileSystem
import me.mumin.android.files.provider.document.resolver.ExternalStorageProviderHacks
import me.mumin.android.files.provider.linux.LinuxFileSystem
import me.mumin.android.files.provider.root.RootStrategy
import me.mumin.android.files.provider.sftp.SftpFileSystem
import me.mumin.android.files.provider.smb.SmbFileSystem
import me.mumin.android.files.storage.DocumentTree
import me.mumin.android.files.storage.FileSystemRoot
import me.mumin.android.files.storage.PrimaryStorageVolume
import me.mumin.android.files.util.StableUriParceler
import me.mumin.android.files.util.asBase64
import me.mumin.android.files.util.readParcelable
import me.mumin.android.files.util.readParcelableListCompat
import me.mumin.android.files.util.toBase64
import me.mumin.android.files.util.toByteArray
import me.mumin.android.files.util.use
        ...
        Parcel.obtain().use { newParcel ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    oldParcel.readInt()
                    val className = oldParcel.readString()
                    if (className == "me.mumin.android.files.storage.SmbServer") {
                        newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                        newParcel.writeString("me.mumin.android.files.storage.SmbServer")
                        ...
                        newParcel.writeString(
                            "me.mumin.android.files.provider.smb.client.Authority"
                        )
                        ...
                        oldParcel.readString()
                        newParcel.writeString(
                            "me.mumin.android.files.provider.smb.client.Authentication"
                        )
...
private fun migratePath1_4_0(oldParcel: Parcel, newParcel: Parcel) {
    ...
    when (className) {
        ...
        "me.mumin.android.files.provider.sftp.SftpPath" ->
            newParcel.writeParcelable(oldParcel.readParcelable<SftpFileSystem>(), 0)
        "me.mumin.android.files.provider.smb.SmbPath" ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "me.mumin.android.files.storage.SftpServer" -> {
                            newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                            newParcel.writeString("me.mumin.android.files.storage.SftpServer")
                            val id = oldParcel.readLong()
                            newParcel.writeLong(id)
                            val customName = oldParcel.readString()
                            newParcel.writeString(customName)
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "me.mumin.android.files.storage.SftpServer" -> {
                            ...
                            val authenticationPasswordOrPrivateKey = oldParcel.readString()
                            newParcel.writeString(authenticationPasswordOrPrivateKey)
                            if (authenticationClassName == "me.mumin.android.files.provider.sftp"
                ...
                Parcel.obtain().use { oldParcel ->
                    ...
                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                        ...
                        repeat(size) {
                            ...
                            oldParcel.setDataPosition(oldPosition)
                            when (className) {
                                "me.mumin.android.files.storage.DocumentManagerShortcut" -> {
                                    newParcel.writeInt(oldParcel.readInt())
                                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                                        oldParcel.readString()
                                        newParcel.writeString(
                                            "me.mumin.android.files.storage" +
