    ...
    fun search(
        ...
        walkFileTreeForSearch(directory, object : FileVisitor<Path> {
            ...
            private fun visit(path: Path) {
                ...
                if (fileName != null) {
                    val nameStr = fileName.toString()
                    val matches = if (query.startsWith("exts:")) {
                        val exts = query.substring(5).split(",")
                        exts.any { nameStr.endsWith(it, true) }
                    } else {
                        nameStr.contains(query, true)
                    }
                    if (matches) {
                        paths.add(path)
                    }
