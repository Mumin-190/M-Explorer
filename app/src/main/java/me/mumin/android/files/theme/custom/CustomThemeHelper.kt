...
import me.mumin.android.files.compat.recreateCompat
import me.mumin.android.files.compat.setThemeCompat
import me.mumin.android.files.compat.themeResIdCompat
import me.mumin.android.files.settings.Settings
import me.mumin.android.files.theme.night.NightModeHelper
import me.mumin.android.files.util.SimpleActivityLifecycleCallbacks
...
object CustomThemeHelper {
    ...
    fun sync(forceRecreate: Boolean = false) {
        for ((activity, baseThemeRes) in activityBaseThemes) {
            val currentThemeRes = activity.themeResIdCompat
            val customThemeRes = getCustomThemeRes(baseThemeRes, activity)
            if (forceRecreate || currentThemeRes != customThemeRes) {
                // Ignore ".Black" theme changes when not in night mode.
                if (!forceRecreate && !NightModeHelper.isInNightMode(activity as AppCompatActivity)
